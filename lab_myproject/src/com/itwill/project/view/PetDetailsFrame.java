...
public class PetDetailsFrame extends JFrame {
	...
	public void initialize() {
		...
		btnUpdate.setFont(new Font("학교안심 바른돋움 R", Font.PLAIN, 15));
		...
		radioBtnMale = new JRadioButton("수컷");
		radioBtnMale.setEnabled(false);
		radioBtnMale.setSelected(true);
		radioBtnMale.setFont(new Font("학교안심 바른돋움 R", Font.PLAIN, 13));
		buttonGroup.add(radioBtnMale);
		radioBtnMale.setBounds(285, 80, 49, 23);
		animalPanel.add(radioBtnMale);
		
		radioBtnfmale = new JRadioButton("암컷");
		radioBtnfmale.setEnabled(false);
		radioBtnfmale.setFont(new Font("학교안심 바른돋움 R", Font.PLAIN, 13));
		...
		animalTypeBox.setEditable(true);
		animalTypeBox.setEnabled(false);
		animalTypeBox.setFont(new Font("학교안심 바른돋움 R", Font.PLAIN, 14));
		animalTypeBox.setBounds(281, 45, 121, 23);
		animalTypeBox.setBackground(Color.WHITE);
		animalTypeBox.setForeground(Color.BLACK);
		...
		textFieldAnimalName.setFont(new Font("학교안심 바른돋움 R", Font.PLAIN, 14));
		textFieldAnimalName.setBounds(65, 45, 128, 21);
		animalPanel.add(textFieldAnimalName);
		textFieldAnimalName.setColumns(10);
		
		textFieldAnimalAge = new JTextField();
		textFieldAnimalAge.setFont(new Font("학교안심 바른돋움 R", Font.PLAIN, 14));
		textFieldAnimalAge.setColumns(10);
		textFieldAnimalAge.setBounds(65, 80, 128, 21);
		animalPanel.add(textFieldAnimalAge);
		
		textFieldAnimalWeight = new JTextField();
		textFieldAnimalWeight.setFont(new Font("학교안심 바른돋움 R", Font.PLAIN, 14));
		textFieldAnimalWeight.setColumns(10);
		textFieldAnimalWeight.setBounds(65, 115, 128, 21);
		...
		textAnimalArea.setFont(new Font("학교안심 바른돋움 R", Font.PLAIN, 15));
		...
		textFieldOwnerName.setFont(new Font("학교안심 바른돋움 R", Font.PLAIN, 14));
		...
		textFieldOwnerContact.setFont(new Font("학교안심 바른돋움 R", Font.PLAIN, 14));
		...
		textFieldOwnerAdress.setFont(new Font("학교안심 바른돋움 R", Font.PLAIN, 14));
		...
		textOwnerArea.setFont(new Font("학교안심 바른돋움 R", Font.PLAIN, 15));
		...
		textFieldUpdated.setBounds(230, 606, 155, 21);
		...
		textFieldCreated.setBounds(46, 606, 155, 21);
		...
		separator_1.setBounds(0, 633, 434, 9);
	...
	private void updateAnimals() {
		...
		try {	
			...
			if (animalAgeStr.length() > 3) {
				JOptionPane.showMessageDialog(
						PetDetailsFrame.this,"나이는 3자리 이하로 입력하세요", "",JOptionPane.WARNING_MESSAGE);
				return;
			}
			if (animalAge < 0 || animalAge > 100) {
				JOptionPane.showMessageDialog(
						PetDetailsFrame.this,"나이는 0 ~ 100 이하의 숫자로 입력하세요", "주의",JOptionPane.WARNING_MESSAGE);
				return;
			}
			
			if (animalWeightStr.length() > 5) {
				JOptionPane.showMessageDialog(
						PetDetailsFrame.this,"체중은 5자리 이하로 입력하세요", "",JOptionPane.WARNING_MESSAGE);
				return;
			}
			
			if (animalWeight < 0 || animalWeight > 500) {
				JOptionPane.showMessageDialog(
						PetDetailsFrame.this,"체중은 0 ~ 500 이하로 입력하세요", "",JOptionPane.WARNING_MESSAGE);
				return;
			}
			
			if (animalName.length() > 3 || ownerName.length() > 3) {
				JOptionPane.showMessageDialog(
						PetDetailsFrame.this,"이름은 3자리 이하로 입력하세요", "",JOptionPane.WARNING_MESSAGE);
				return;
			}
